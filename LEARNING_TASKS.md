...
| 26 | パスワードを安全にハッシュ化する（bcrypt / argon2） | 未対応 |
| 27 | 一般的な攻撃からAPIを守る（XSS / CSRF / SQLインジェクション） | 未対応 |
| 28 | レートリミットやスロットリングを実装する | 未対応 |
| 29 | HTTPSと暗号化を利用する | 未対応 |

### インフラ・スケーラビリティ（30〜34）

| No | 項目 | 状態 |
|---|---|---|
| 30 | Dockerを使ったバックエンドデプロイを学ぶ | ✅ 済み |
| 31 | CI/CDパイプラインを理解する | 未対応 |
| 32 | メッセージキューを使う（Kafka / RabbitMQ / SQS） | 未対応 |
| 33 | バックグラウンドジョブやワーカーを実装する | 未対応 |
| 34 | マイクロサービスアーキテクチャの基礎を学ぶ | ✅ 済み |
